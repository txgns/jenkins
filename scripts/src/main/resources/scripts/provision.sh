...
then
  ...
  echo "java \"-DMASTER_ENDPOINT=http://localhost:${MASTER_PORT}\" \"-DJENKINS_HOME=${MASTER_HOME}\" \"-DMASTER_HOME=${MASTER_HOME}\" \"-DMASTER_METANECTAR_ENDPOINT=${MASTER_METANECTAR_ENDPOINT}\" \"-DMASTER_GRANT_ID=${MASTER_GRANT_ID}\" -jar "${WORK}/../jenkins-war.war" --httpPort=${MASTER_PORT} --ajp13Port=-1 &> \"${MASTER_HOME}.log.txt\" &" >> "${MASTER_HOME}.start"
